<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 095 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 095 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 提取决定身份的节点、方向和动作 → 用 visual shorthand 压缩对象 → 以主图解建立唯一视觉锚点 → 用辅助图示、引导线、对齐与错位形成阅读路径 → 以黑色技术线承载信息、红色 spot color 标记重点 → 用纸白、油墨颗粒与套印偏差完成编辑图版 → 加入少量图解式文字
```

## 成品中最容易识别的风格特点

- 黑色或深墨色细线是主体，红色只在关键结构、路径、信号、关键词或象征节点出现。
- 模块化但不僵硬宫格：主图解明确，辅助图示少量、错位、对齐并保留大面积留白。
- 线条兼具手绘技术图与早期印刷图版质感，保留轻微油墨颗粒、纸纤维、套印偏差与 overprint。
- 文字是图解系统的一部分，以窄体无衬线活字印刷感融入图标、引导线和编号关系。
- 拒绝写实插画、复杂透视、彩色信息图、科技 UI、卡通图标、装饰堆积和模板化复古海报。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-095-prompt.en.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 表现性黑墨线 · 自由轮廓 · 几何／潦草排线 · 负形构图 · 纸白 · 哲思克制 |
| **[xxd-panel-093](https://github.com/nevertoday/xxd-panel-093)** | 几何平涂 · 扁平空间 · 明快取色 · 手绘边缘 · 哑光水粉 · 现代主义编辑插画 |
| **[xxd-panel-095](https://github.com/nevertoday/xxd-panel-095)** | 技术细线 · 图解简化 · 黑红专色 · 模块化信息 · 大留白 · 旧式印刷肌理 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–095） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
