<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 095 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 095 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

装飾的な挿絵は写真に似せられても、何が主体を識別させ、各要素がどう関係するかまでは説明できません。

**Panel 095** は写真をミッドセンチュリーの図解的編集図版へ圧縮します。黒い技術細線で同一性と構造を説明し、最重要の節点・経路・信号・語だけを赤い特色で示し、モジュール化した絵記号と暖かな紙の大きな余白で複雑さを明快な読解経路に変えます。

### 向いている用途

- 複雑な対象を、外輪郭・構造節点・方向・動作・象徴関係だけへ圧縮したい場合
- 硬直したグリッドなしで、技術マニュアル、交通図解、初期情報デザインの知的な編集性を求める場合
- 黒赤二色印刷、細いモダニズム注記、インク粒子、紙繊維、版ずれ、オーバープリントを生かしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 095 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 黒い技術細線 · 絵記号的簡略化 · 赤の特色 · モジュール図解 · 暖色紙 · 旧式印刷
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 写真を装飾するのでなく、明快に説明する人間味あるミッドセンチュリー編集図解 |
| **識別しやすい特徴** | 黒い技術細線、限定的な赤の特色、モジュール絵記号、暖かな大余白、わずかな旧印刷の揺らぎ |
| **入力の尊重** | 同一性、輪郭、構造、姿勢、方向、物語関係を最少で正確な視覚情報へ圧縮 |
...
```text
主体・輪郭・構造・姿勢・物語関係を分析 → 識別を決める輪郭・節点・方向・動作・象徴へ簡略化 → 明確な主図解を一つ置く → 必要最小限の補助図を加える → 尺度差・ずれ・整列・引出線で読解経路を作る → 暖色紙に黒い技術細線で描く → 要所だけ赤い特色で示す → 小さな細身モダニズム注記を統合 → 粒子・紙繊維・版ずれ・オーバープリントをわずかに残す
```

## 完成作の特徴

- 細く均一で抑制された黒または濃墨線に、輪郭・構造・情報の大半を担わせる。
- 朱赤、珊瑚赤、印刷赤は、決定的な構造、動作節点、経路、信号、領域、語、象徴だけに特色として使う。
- 主役は一つに絞り、補助図は説明だけを担い、第二の物語中心を作らない。
- 小さな細身サンセリフを、アイコン、引出線、赤印、余白と整列する編集注記として扱う。
- 平面空間と軽い紙繊維・インク粒子・版ずれ・重ね刷りを保ち、グラデーション、多色情報図、Tech UI、硬直グリッド、装飾過多、偽古フィルター、テンプレート復古ポスターを避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-095-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 095 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-095 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-095 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-095 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-095 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 095 を適用し、共通設定は一度だけ解決
/xxd-panel-095 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-095`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-095-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-095-prompt.zh-CN.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 表現的な黒墨線 · 自由輪郭 · 幾何／スクリブル排線 · 負形 · 紙白 · 哲学的抑制 |
| **[xxd-panel-093](https://github.com/nevertoday/xxd-panel-093)** | 幾何平塗り · 平面空間 · 鮮明な写真由来色 · 手描きの縁 · マットなガッシュ · ミッドセンチュリー編集 |
| **[xxd-panel-095](https://github.com/nevertoday/xxd-panel-095)** | 黒い技術細線 · 絵記号的簡略化 · 赤の特色 · モジュール図解 · 暖色紙 · 旧式印刷 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–095） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
