<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 095 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 095이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

장식적 삽화는 사진과 닮을 수 있지만 무엇이 대상을 식별하게 하는지, 요소들이 어떻게 연결되는지는 설명하지 못합니다.

**Panel 095**는 사진을 미드센추리 도식형 편집 도판으로 압축합니다. 검은 기술 세선이 정체성과 구조를 설명하고, 가장 중요한 결절·경로·신호·단어만 붉은 별색으로 표시하며, 모듈형 픽토그램과 넓은 웜 페이퍼 여백이 복잡함을 명료한 읽기 경로로 바꿉니다.

### 이런 경우에 적합합니다

- 복잡한 대상을 핵심 윤곽, 구조 결절, 방향, 동작, 상징 관계로 압축하고 싶을 때
- 경직된 격자 없이 기술 매뉴얼, 교통 도해, 초기 정보 디자인의 지적인 편집 감각이 필요할 때
- 흑적 2도 인쇄, 가는 모더니즘 주석, 잉크 입자, 종이 섬유, 미세한 판 어긋남과 오버프린트를 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 095로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 검은 기술 세선 · 픽토그램식 단순화 · 붉은 별색 · 모듈형 도해 · 웜 페이퍼 · 빈티지 인쇄
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 사진을 꾸미는 데 그치지 않고 명료하게 설명하는 인간적인 미드센추리 편집 도해 |
| **대표 특징** | 가는 검은 기술선, 절제된 붉은 별색, 모듈형 픽토그램, 넓은 웜 페이퍼 여백, 미세한 옛 인쇄 흔적 |
| **입력 존중 방식** | 정체성·윤곽·구조·자세·방향·서사 관계를 가장 적고 정확한 시각 정보로 압축 |
...
```text
주제·윤곽·구조·자세·서사 관계 분석 → 식별을 결정하는 윤곽·결절·방향·동작·상징으로 단순화 → 명확한 주 도해 하나를 배치 → 꼭 필요한 보조 도해만 추가 → 크기 차·엇갈림·정렬·안내선으로 읽기 경로 구성 → 웜 페이퍼 위에 검은 기술 세선으로 표현 → 핵심만 붉은 별색으로 강조 → 작은 세로형 모더니즘 주석 통합 → 잉크 입자·종이 섬유·판 어긋남·오버프린트를 은은하게 유지
```

## 완성작의 특징

- 가늘고 균일하며 절제된 검정 또는 짙은 먹선이 윤곽·구조·정보 대부분을 담당합니다.
- 주홍, 코럴 레드, 인쇄 적색은 결정적 구조, 동작 결절, 경로, 신호, 영역, 핵심어, 상징에만 별색으로 씁니다.
- 시각 중심은 하나만 두고 보조 도해는 설명만 하며 두 번째 서사 중심을 만들지 않습니다.
- 작은 세로형 산세리프 문자를 아이콘, 안내선, 붉은 표식, 여백과 정렬되는 편집 주석으로 다룹니다.
- 평면 공간과 미세한 종이 섬유·잉크 입자·판 어긋남·오버프린트를 유지하고, 그라데이션·다색 인포그래픽·테크 UI·경직된 격자·장식 과잉·가짜 앤티크 필터·템플릿 복고 포스터를 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-095-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 095의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-095 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-095 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-095 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-095 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-095 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-095 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 095 적용, 공통 설정은 한 번만 확정
/xxd-panel-095 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-095`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-095-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-095-prompt.zh-CN.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 표현적 검은 먹선 · 자유 윤곽 · 기하／스크리블 해칭 · 네거티브 스페이스 · 종이 흰색 · 철학적 절제 |
| **[xxd-panel-093](https://github.com/nevertoday/xxd-panel-093)** | 기하 평면색 · 평면 공간 · 선명한 원본 색 · 손그림 가장자리 · 무광 과슈 · 미드센추리 편집 |
| **[xxd-panel-095](https://github.com/nevertoday/xxd-panel-095)** | 검은 기술 세선 · 픽토그램식 단순화 · 붉은 별색 · 모듈형 도해 · 웜 페이퍼 · 빈티지 인쇄 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–095) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
